...
<head>
    ...
    <% var title=page.title; if (is_archive()){ title=__('Archive'); if (is_month()){ title +=' | ' + page.year + '/' +
        page.month; } else if (is_year()){ title +=' | ' + page.year; } } else if (is_category()){ title=__('Category')
        + ' | ' + page.category; } else if (is_tag()){ title=__('Tag') + ' | ' + page.tag; } %>
        <title>
            <% if (title){ %>
                <%= title %> | <% } %>
                        <%= config.title %>
        ...
        <meta name="description"
            content="<%= page.description || page.excerpt || config.description || config.title %>">
                    ...
                    <% if (config.aomori_search_algolia){ %>
                        <%- algolia_search_config() %>
                            <% } %>
                                ...
                                <% if (config.rss){ %>
                                    <link rel="alternate" href="<%= url_for(config.rss) %>" title="<%= config.title %>"
                                        ...
                                        <% if (config.favicon){ %>
                                            ...
                                            <% } %>
                                                ...
                                                <% if (config.aomori_favicon) { %>
                                                    <link rel="icon" href="<%= config.aomori_favicon %>">
                                                    <% } %>
                                                        ...
                                                        <% if (config.aomori_gitalk){ %>
                                                            ...
                                                            <% } %>
                                                                ...
                                                                <% if (config.aomori_disqusjs){ %>
                                                                    <link rel="stylesheet"
                                                                        href="https://unpkg.com/disqusjs@1.3/dist/disqusjs.css">
                                                                    <% } %>
